...
<body>
	<div id="topbar">
		...
		<center><table class="box1" cellpadding="3" cellspacing="3" border="0" width="1200">
			<tr>
				<td width="623">
					<h1>Exluir post <?php echo $result['post_title']; ?>?</h1>
				</td>
			</tr>
		</table>
		<?php
		...
		<table class="box22" cellpadding="8" cellspacing="8" border="0" width="1200">
			<tr>
				...
				<?php
					...
					if(isset($_POST['sim'])){
						$sqlExcluirPost = $dbconn->prepare("DELETE FROM `forum_post` WHERE `post_author`=:postauthor AND `post_id`=:idpost");
						$sqlExcluirPost->bindParam(':postauthor', $result['post_author'], PDO::PARAM_INT);
						$sqlExcluirPost->bindParam(':idpost', $postidurl, PDO::PARAM_INT);
						if($sqlExcluirPost->execute()){
							header('Location: viewforum.php?fid='.$resultForum['forum_id']);
						}
					}
				?><br /><br />
				<form method="POST" action="">
					<input type="submit" name="sim" value=" Sim " />&nbsp;&nbsp;
					<a href="viewtopic.php?pid=<?php echo $postidurl; ?>"> N&atilde;o </a>
				</form><br />
				</td>
			</tr>
		</table><br />
